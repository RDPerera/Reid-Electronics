(function(Eventgallery, jQuery){
    ...
    Eventgallery.ScalePriceEditor.prototype.initialize = function() {
        ...
        this.$buttonAddLine.on('click', jQuery.proxy(function(e){
            e.preventDefault();
            let quantity = 2;
            let elements = this.$linesElement.find('.' + Eventgallery.ScalePriceEditorLine.lineCssClass);
            if (elements.length > 0) {
                quantity = parseInt(this.$linesElement.find('.' + Eventgallery.ScalePriceEditorLine.lineCssClass).last().data('line').quantity) + 1;
            }
