...
class plggeocodefactoryPlg_geofactory_gw_jc30 extends GeofactoryPluginHelper{
	...
	protected $custom_latitude	= true ; 				// bool
	...
	public function getListQueryBackGeocode($type, $filters, $papa, $vAssign){
		...
		// primo on importe les content qui ne sont pas dans ma base
		$query = $db->getQuery(true);
		$query->select('id');
		$query->from($db->quoteName('#__content')); // select id from test3_content where id NOT IN(SELECT id_content FROM test3_geofactory_contents WHERE type='MS_JC')
		$query->where('id NOT IN(SELECT id_content FROM '.$db->quoteName($this->plgTable).' WHERE type='.$db->Quote($this->gatewayOption).')');
		$db->setQuery($query);
    	...
    	if(is_array($res) AND count($res)>0){
			...
			foreach ($res as $art)
				$query->values(implode(',', array($db->quote(''),$db->quote($this->gatewayOption), (int)$art->id, $db->quote(''), (float)$this->defEmptyLat, (float)$this->defEmptyLng)));
			$db->setQuery($query);
		    $result = $db->execute();
    	}
		...
		// articles already geocoded (in GF table)
		$query->select(
			$papa->getState(
				'list.select',
				'a.id 		AS item_id,'.
	...
	public function setItemCoordinates($type, $id, $vCoord, $vAssign){
		// !!! on travaille avec le id de la table... pas celui de l'article !!!! --- SAUF en cas de geocode de nouvel éléement par meatadata.... 
		if (!$this->_isInCurrentType($type))
	...
	public function getItemPostalAddress($type, $id, $vAssign){
		...
		if 			(isset($vAssign["field_latitude"]) AND $vAssign["field_latitude"]==-2){
			$adresse = $this->_getItemFromArticle($id,'metakey');
			if (strlen(trim($adresse))<1)
				return array($type, $add) ;

			$add["field_city"] = $adresse ;
		}else if 	(isset($vAssign["field_latitude"]) AND $vAssign["field_latitude"]==-3){
	...
	public function getCustomFieldsCoord($typeList, &$options){
		...
		// use the users saved fiels in custom database
		$tmp		= new stdClass() ;
		$tmp->value	= '' ;
		$tmp->text	= "Users manualy saved address with button" ;
		$options[] 	= $tmp ;
	...
	public function _getItemFromArticle($id, $field, $purejoomla=false){
		...
		$res = $db->loadObjectList();
		if ($db->getErrorNum()) { trigger_error("_getItemFromArticle : DB reports: ".$db->stderr(), E_USER_WARNING);}
