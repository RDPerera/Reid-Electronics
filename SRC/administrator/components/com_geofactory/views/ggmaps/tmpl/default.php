...
<form action="<?php echo JRoute::_('index.php?option=com_geofactory&view=ggmaps'); ?>" method="post" name="adminForm" id="adminForm">
			...
			<table class="table table-striped">
				...
				<?php foreach ($this->items as $i => $item) :
					...
					<tr class="row<?php echo $i % 2; ?>">
						...
						<td class="center">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'ggmaps.', $canChange);?>
						</td>
						<td class="nowrap has-context">
							<div class="pull-left">
								<?php if ($item->checked_out) : ?>
									<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'ggmaps.', $canCheckin); ?>
								<?php endif; ?>
								<?php if ($canEdit) : ?>
									<a href="<?php echo JRoute::_('index.php?option=com_geofactory&task=ggmap.edit&id='.(int) $item->id); ?>">
							...
							<div class="pull-left">
								<?php
									...
									JHtml::_('dropdown.edit', $item->id, 'ggmap.');
									JHtml::_('dropdown.divider');
									if ($item->state) :
										JHtml::_('dropdown.unpublish', 'cb' . $i, 'ggmaps.');
									else :
										JHtml::_('dropdown.publish', 'cb' . $i, 'ggmaps.');
									endif;

									JHtml::_('dropdown.divider');

									if ($archived) :
										JHtml::_('dropdown.unarchive', 'cb' . $i, 'ggmaps.');
									else :
										JHtml::_('dropdown.archive', 'cb' . $i, 'ggmaps.');
									endif;

									if ($item->checked_out) :
										JHtml::_('dropdown.checkin', 'cb' . $i, 'ggmaps.');
									endif;

									if ($trashed) :
										JHtml::_('dropdown.untrash', 'cb' . $i, 'ggmaps.');
									else :
										JHtml::_('dropdown.trash', 'cb' . $i, 'ggmaps.');
									endif;
